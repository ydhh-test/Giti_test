# -*- coding: utf-8 -*-
"""
rule11: 纵向细沟 & 纵向钢片 检测中间层（需求11）

功能:
- 接收 postprocessor 的调用
- 循环 center_inf / side_inf 目录中的所有图片
- 调用 detect_longitudinal_grooves() 进行检测与评分
...
def process_longitudinal_grooves(task_id: str, conf: dict) -> Tuple[bool, dict]:
    """
    纵向细沟 & 纵向钢片检测主入口。
    ...
    conf : dict
        ...
        - ``groove_width_mm`` (*float*, 可选): 纵向线条名义宽度（mm）。
        - ``pixel_per_mm`` (*float*, 可选): 像素密度（px/mm）。
        - ``min_width_offset_px`` (*int*, 可选): 宽度下限偏移（像素）。
        - ``edge_margin_ratio`` (*float*, 可选): 左右边缘排除比例。
        - ``min_segment_length_ratio`` (*float*, 可选): 连续线段最小长度比例。
        - ``max_angle_from_vertical`` (*float*, 可选): 最大倾斜角（度）。
...
def _process_single_image(
    ...
    try:
        ...
        # 组装检测参数（conf 中未提供的项由算法函数内部使用默认值）
        kwargs: Dict[str, Any] = {}
        ...
        if "min_width_offset_px" in conf:
            kwargs["min_width_offset_px"] = conf["min_width_offset_px"]
        if "edge_margin_ratio" in conf:
            kwargs["edge_margin_ratio"] = conf["edge_margin_ratio"]
        if "min_segment_length_ratio" in conf:
            kwargs["min_segment_length_ratio"] = conf["min_segment_length_ratio"]
        # 配置类字段名 → 算法参数名的映射
        if "max_angle_from_vertical" in conf:
            kwargs["max_angle_deg"] = conf["max_angle_from_vertical"]

        score, details = detect_longitudinal_grooves(img, image_type, **kwargs)
        ...
        return True, {
            ...
            "groove_widths":    details["groove_widths"],
