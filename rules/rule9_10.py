# -*- coding: utf-8 -*-
"""
rule9_10: 横向钢片检测中间层（需求9 & 需求10）

功能:
- 接收 postprocessor 的调用
- 循环 center_inf / side_inf 目录中的所有图片
- 调用 detect_horizontal_sipes() 进行检测与评分
- 保存调试标注图和汇总 results.json

目录关系:
- 输入：.results/task_id_{task_id}/center_inf/*.png
        .results/task_id_{task_id}/side_inf/*.png
- 输出：.results/task_id_{task_id}/detect_horizontal_sipes/center/
        .results/task_id_{task_id}/detect_horizontal_sipes/side/
...
logger = get_logger("rule9_10")

# 输出根目录名
_DETECTOR_DIR = "detect_horizontal_sipes"
...
def process_horizontal_sipes(task_id: str, conf: dict) -> Tuple[bool, dict]:
    """
    横向钢片检测主入口。
    ...
    conf : dict
        ...
        - ``sipe_width_mm`` (*dict*, 可选): 各类型 POC 钢片宽度（mm）。
        - ``sipe_width_range_mm`` (*list*, 可选): 钢片宽度范围 [min, max]（mm）。
        - ``groove_min_width_mm`` (*dict*, 可选): 各类型横沟最小宽度（mm）。
        - ``pixel_per_mm`` (*float*, 可选): 像素密度（px/mm）。
        - ``sipe_count_max`` (*dict*, 可选): 各类型允许的最大钢片数。
        - ``position_tolerance`` (*float*, 可选): 位置均分偏差容忍比例。
...
def _process_single_image(
    ...
    """
    from algorithms.detection.sipe_detection import detect_horizontal_sipes
    ...
    try:
        ...
        # 组装检测参数
        kwargs: Dict[str, Any] = {}
        if "sipe_width_mm" in conf:
            kwargs["sipe_width_mm"] = conf["sipe_width_mm"]
        if "sipe_width_range_mm" in conf:
            kwargs["sipe_width_range_mm"] = conf["sipe_width_range_mm"]
        if "groove_min_width_mm" in conf:
            kwargs["groove_min_width_mm"] = conf["groove_min_width_mm"]
        if "pixel_per_mm" in conf:
            kwargs["pixel_per_mm"] = conf["pixel_per_mm"]
        if "sipe_count_max" in conf:
            kwargs["sipe_count_max"] = conf["sipe_count_max"]
        if "position_tolerance" in conf:
            kwargs["position_tolerance"] = conf["position_tolerance"]

        score, details = detect_horizontal_sipes(img, image_type, **kwargs)

        if score is None:
            return False, {
                "file": fpath.name,
                "status": "failed",
                "err_msg": details.get("err_msg", "检测失败"),
            }
        ...
        return True, {
            ...
            "sipe_count":       details["sipe_count"],
            "sipe_positions":   details["sipe_positions"],
            "groove_count":     details["groove_count"],
            "groove_positions": details["groove_positions"],
            "is_valid":         details["is_valid"],
            "score_req9":       details["score_req9"],
            "score_req10":      details["score_req10"],
            "total_score":      score,
