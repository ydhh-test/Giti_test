# -*- coding: utf-8 -*-
"""
rule8_14 单元测试 - 横沟检测中间层（需求8 & 需求14）
"""
...
from rules.rule8_14 import (
    process_transverse_grooves,
...
class TestRule8_14:
    ...
    # 与 rule8_14._DETECTOR_DIR 保持一致
    DETECTOR_DIR   = "detect_transverse_grooves"
    ...
    def test_process_single_image_ok(self, tmp_path):
        ...
        assert 0.0 <= result["total_score"] <= 6.0
    ...
    def test_process_single_image_side_ok(self, tmp_path):
        ...
        assert ok is True
        assert result["rib_type"] == "RIB2/3/4"
        assert 0.0 <= result["total_score"] <= 6.0
    ...
    # ------------------------------------------------------------------
    # process_transverse_grooves — 边界条件
    # ------------------------------------------------------------------
    ...
    def test_nonexistent_input_dir_returns_empty_stats(self):
        ...
        flag, details = process_transverse_grooves(
            self.TEST_TASK_ID,
    ...
    def test_custom_input_dirs_override(self, tmp_path):
    ...
    # ------------------------------------------------------------------
    # process_transverse_grooves — 完整流程（使用真实测试数据集）
    # ------------------------------------------------------------------

    def _copy_dataset(self, src_subdir: str, dst_subdir: str) -> int:
        """
        将 tests/datasets/{TASK_ID}/{src_subdir}/ 拷贝到
    ...
    def test_full_center_and_side(self):
        ...
        flag, details = process_transverse_grooves(self.TEST_TASK_ID, self._conf())
        ...
        if n_center > 0:
            ...
            # 每条记录包含必要字段
            required = {"file", "status", "groove_count", "intersection_count",
    ...
    def test_score_range_on_real_images(self):
        """真实图片上每张的 total_score 在 [0, 6] 范围内"""
        n = self._copy_dataset("center_inf", "center_inf")
        if n == 0:
            pytest.skip("center_inf 测试数据集不存在")

        _, details = process_transverse_grooves(
            self.TEST_TASK_ID,
            self._conf(input_dirs={"center_inf": "center"})
        )
        center_out = self.TEST_OUTPUT_BASE / self.DETECTOR_DIR / "center"
        ...
        for entry in data:
            if entry["status"] == "ok":
                assert 0.0 <= entry["total_score"] <= 6.0, (
