...
API 注意：detect_pattern_continuity() 仍返回 (score, details)（暂保留 core 内嵌评分）
...
class TestPatternContinuityFull(unittest.TestCase):
    ...
    API：返回 (score, details)，其中 score 由 config.score 决定。
    """
    ...
    def test_score_is_int_or_none(self):
        """score 应为 int 或 None（出错时）"""
        img = _gray_image()
        score, _ = self._run(img)
    ...
    def test_config_from_dict(self):
        """from_dict 应正确覆盖字段"""
        from src.core.detection.pattern_continuity import PatternContinuityConfig
        cfg = PatternContinuityConfig.from_dict({"score": 5, "edge_height": 8})
        self.assertEqual(cfg.score, 5)
    ...
    def test_center_inf_no_crash(self):
        ...
        for p in self._iter_images("center_inf"):
            with self.subTest(img=p.name):
                score, details = self._run(p)
                self.assertIn("is_continuous", details)

    def test_center_inf_score_type(self):
        """center_inf → score 为整数"""
        for p in self._iter_images("center_inf"):
            with self.subTest(img=p.name):
                score, details = self._run(p)
                self.assertIsInstance(score, int)

    def test_center_inf_score_range(self):
        """center_inf → score 在 [0, 10] 范围内"""
        for p in self._iter_images("center_inf"):
            with self.subTest(img=p.name):
                score, _ = self._run(p)
