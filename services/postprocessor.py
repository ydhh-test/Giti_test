...
if __name__ == "__main__":
    # 读取配置
    rib_count = CONFIG.get('rib', {}).get('count', 5)
    max_per_mode = CONFIG.get('generation', {}).get('max_per_mode', 10)

    print(f"RIB 数量: {rib_count}")
    print(f"每种模式生成上限: {max_per_mode}")

    # 读取图片
    print("\n[1/4] 读取图片...")
    center_images, side_images, image_names = load_images_from_directories()

    print(f"  - Center 图片数量: {len(center_images)}")
    print(f"  - Side 图片数量: {len(side_images)}")

    # 检查数量限制
    side_min = CONFIG.get('image_limits', {}).get('side', {}).get('min', 1)
    side_max = CONFIG.get('image_limits', {}).get('side', {}).get('max', 10)
    center_min = CONFIG.get('image_limits', {}).get('center', {}).get('min', 2)
    center_max = CONFIG.get('image_limits', {}).get('center', {}).get('max', 20)

    if len(side_images) < side_min or len(side_images) > side_max:
        print(f"[警告] Side 图片数量 ({len(side_images)}) 不在限制范围 [{side_min}, {side_max}] 内")
    if len(center_images) < center_min or len(center_images) > center_max:
        print(f"[警告] Center 图片数量 ({len(center_images)}) 不在限制范围 [{center_min}, {center_max}] 内")

    # 读取生成模式配置
    generation_mode = CONFIG.get('generation', {}).get('mode', 'both')
    valid_modes = ['asymmetric', 'rotate180', 'mirror', 'mirror_shifted', 'all_symmetry', 'both', 'random']

    if generation_mode not in valid_modes:
        print(f"[警告] 无效的生成模式 '{generation_mode}'，已切换为 'both'")
    ...
    if generation_mode == 'random':
        # 随机选择一种模式生成
        print("\n[随机模式] 随机选择生成...")
        modes_to_try = ['asymmetric', 'rotate180', 'mirror', 'mirror_shifted']
        chosen_type = random.choice(modes_to_try)
        print(f"  - 随机选中的模式: {chosen_type}")

        results = generate_layout_images(
            center_images, side_images, rib_count, symmetry_type=chosen_type
        )
        ...
        print("\n[2/4] 生成非对称模式图片...")
        results = generate_layout_images(
            center_images, side_images, rib_count, symmetry_type="asymmetric"
        )
        all_results.extend(results)

        print("\n[3/4] 生成对称模式图片 (rotate180/mirror/mirror_shifted)...")
        symmetry_results = generate_layout_images(
            center_images, side_images, rib_count, symmetry_type="all_symmetry"
        )
        all_results.extend(symmetry_results)

    elif generation_mode == 'all_symmetry':
        # 生成所有对称模式
        print("\n[2/4] 生成对称模式图片 (rotate180/mirror/mirror_shifted)...")
        results = generate_layout_images(
            center_images, side_images, rib_count, symmetry_type="all_symmetry"
        )
        all_results.extend(results)

    elif generation_mode == 'asymmetric':
        # 仅生成非对称模式
        print("\n[2/4] 生成非对称模式图片...")
        results = generate_layout_images(
            center_images, side_images, rib_count, symmetry_type="asymmetric"
        ...
        # 生成指定的对称模式 (rotate180/mirror/mirror_shifted)
        print(f"\n[2/4] 生成对称模式图片 ({generation_mode})...")
        results = generate_layout_images(
            center_images, side_images, rib_count, symmetry_type=generation_mode
        )
        all_results.extend(results)

    # 保存结果
    print("\n[4/4] 保存结果...")
    print(f"  - 共生成 {len(all_results)} 张图片")
